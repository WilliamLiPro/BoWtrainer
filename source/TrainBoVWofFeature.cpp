...
bool TrainBoVWofFeature::getSamplesOfFeature(int data_type,int camera_id)
{
	...
	if(data_type==0)//选择摄像头
	{
		...
		{
			MessageBoxA(0, "无法打开摄像头: 请尝试其他摄像头","error", MB_ICONERROR);
			return false;
		}
	}
	if(data_type==1)//选择图片集
	{
		...
		string picture_file_path=picture_file_path_;
		cout<<"打开文件... "<<picture_file_path<<endl;
		out_message_="打开文件... "+picture_file_path+" \n";
		...
		if(!capture.isOpened()) // 打开图片路径
		{
			...
			if(!capture.isOpened()) // 打开图片路径
			{
				MessageBoxA(0, "无法打开指定文件","error", MB_ICONERROR);
				return false;
			}
		}
	}
	...
	//选择是否继续上一次的采样
	int result=MessageBoxA(0, "是否继续上一次的采样？","选择", MB_YESNOCANCEL);
	string feature_nm=descriptor_type_+" samples.dat";
	string file_name="feature samples/"+feature_nm;
	ios_base::openmode open_mode;
	int lines_of_file=0;

	if(result==IDCANCEL)	//用户取消
	{
		return true;
	}
	...
	if(result==IDYES)
	{
		...
		if(!infile)//文件不存在
		{
			...
			if(!infile.is_open())
			{
				cout<<"文件无法打开:"<<file_name<<endl;
				out_message_="文件无法打开： "+file_name+" \n";
	...
	while(1)
	{
		...
		char out_msg[20];
		itoa(total_sp+lines_of_file,out_msg,10);
		out_message_=out_msg;
		out_message_="当前总的特征数目为: "+out_message_+" \n";

		if(cv::waitKey(5)>0||stop_==true||total_sp+lines_of_file>1000000)//用户选择，或特征数目过多，强制退出
		{
			out_message_="退出采样 \n";
			break;
		}
	...
	//存储采样结果
	out_message_="正在保存采样结果... \n";

	int feature_n=samples_of_feature_.size();		//总的特征个数

	string total_ftn;
	char total_ft_nb[20];
	itoa(feature_n,total_ft_nb,10);
	total_ftn=total_ft_nb;
	...
	for(int i=0;i<feature_n;i++)
	{
		...
		if(i%500==1)
		{
			...
			itoa(i,out_msg,10);
			out_message_=out_msg;
			out_message_="共有 "+total_ftn+" 个特征，已经保存: "+out_message_+" 个 \n";
		}
	}
...
bool TrainBoVWofFeature::setBoVWlevelsAndBranches(int tree_levels,int branches)
{
	...
	if(tree_levels<2||tree_levels>12)
	{
		...
		out_message_="error: 输入K-D树层数不在有效范围内 2~12: \n";
	...
	if(branches<2||branches>10)
	{
		...
		out_message_="error: 输入K-D树分支数不在有效范围内 2~10: \n";
...
bool TrainBoVWofFeature::buildBagofVisvalWord()
{
	...
	int result=MessageBoxA(0, "是否开始训练视觉词典","选择", MB_YESNOCANCEL);
	if(result==IDCANCEL)	//用户取消
	{
		return true;
	}
	...
	cout<<"文件名："<<descriptor_type_<<" BoVW.dat"<<endl;

	out_message_="词典构造函数，BoVW训练结果存于工程目录：BoVW 文件夹下 \n";		//输出界面显示消息
	out_message_=out_message_+"文件名："+descriptor_type_+" BoVW.dat \n";
	...
	string file_nm=descriptor_type_+" samples.dat";
	string file_name_in="feature samples/"+file_nm;
	cout<<"正在打开文件: "<<file_name_in<<" ..."<<endl;
	out_message_="正在打开文件: "+file_name_in+" ... \n";
	...
	if(!infile)//文件不存在
	{
		...
		if(!infile.is_open())
		{
			...
			out_message_="无法打开文件:"+file_name_in+" \n";
	...
	while(!infile.eof())//一直读到文件末尾
	{
		...
		if(feature_nums%1000==0)
		{
			...
			char str[20];
			itoa(feature_nums,str,10);
			out_message_=str;
			out_message_="获取第 "+out_message_+" 个特征样本 \n";
		}
	...
	if(total_center_nums*10>feature_nums)
	{
		...
		out_message_="样本过少或BoVW末端分支过多，请继续采样或减小k-d树分支树及层数 \n";
		return false;
	}
	...
	for(int i=0;i<tree_levels;i++)
	{
		...
		if(i==0)//如果是第1层
		{
			...
			itoa(i,str,10);
			out_message_=str;
			out_message_=out_message_+" 层k-means均值聚类 \n";
			...
			for(int j=0;j<k_means_result.size();j++)
			{
			...
			int sample_belong_n=samples_belones_result.size();

			for(int j=0;j<sample_belong_n;j++)
			{
		...
		int samples_belones_upper_level_size=samples_belones_upper_level.size();
		for(int j=0;j<samples_belones_upper_level_size;j++)
		{
			if(stop_==true)	//用户终止
			{
				out_message_="训练终止 \n";
			...
			itoa(i,str,10);
			out_message_=str;
			out_message_=out_message_+" 层k-means均值聚类,分支：";
			itoa(j,str,10);
			string branch_id=str;
			out_message_=out_message_+branch_id+" \n";
			...
			//将k-means聚类中心存入分支树数据结构中
			for(int k=0;k<k_means_result.size();k++)
			{
				bag_of_visual_word_.kd_tree_data.push_back(k_means_result[k]);
			}

			//将样本分组结果存于samples_belones_upper_level
			for(int k=0;k<samples_belones_result.size();k++)
			{
	...
	result=MessageBoxA(0, "是否储存本次 BoVW 训练结果?","选择", MB_YESNOCANCEL);
	...
	if(result==IDNO)//退出buildBagofVisvalWord
	{
		...
		out_message_=" 退出视觉词典训练程序... \n";
		Sleep(500);
		return false;
	}
	cout<<"储存 BoVW  ..."<<endl;
	out_message_=" 储存 BoVW  ... \n";

	//3.将聚类结果存入dat文档
	string save_nm=descriptor_type_+" BoVW.dat";
...
bool TrainBoVWofFeature::detectFeature()
{
	...
	cv::drawKeypoints(frame_now_,feature_keypoints,draw_keypoints_,CV_RGB(80,160,80), 0);//绘制特征

	if (draw_keysampling_==true)
	{
		cv::imshow( "keypoints", draw_keypoints_ );
	}
	...
	{
		if(descriptor_type_=="SURF"||descriptor_type_=="SIFT")		//连续描述特征
		{
		...
		{
			uchar* ft_descriptor_ptr=feature_descriptor.ptr<uchar>(i);
			for(int j=0;j<feature_descriptor.cols;j++)
			{
				feature_descriptor_ready[j]=ft_descriptor_ptr[j];
			}
