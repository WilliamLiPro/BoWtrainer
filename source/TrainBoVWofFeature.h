...
public:
	...
	/*	特征采集与BoVW计算
		...
		camera_id	启动的摄像头序号*/
	bool getSamplesOfFeature(int data_type,int camera_id);//特征采样

	bool setFeatureType(string feature_type,string descriptor_type);//设置特征类别
	...
	cv::Mat draw_keypoints_;
	bool draw_keysampling_;	//是否绘制采样结果

	bool stop_;

	string out_message_;		//输出界面显示特征
	string picture_file_path_;	//图片样本路径
private:
	//输入采集图像
	...
	string feature_type_;				//特征类型
	string descriptor_type_;			//描述子类型
	cv::Ptr<cv::FeatureDetector> feature_detector_;//特征检测器
	cv::Ptr<cv::DescriptorExtractor> feature_extractor_;//特征描述器
